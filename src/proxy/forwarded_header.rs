//! Copied from <https://github.com/svenstaro/proxyboi/blob/master/src/forwarded_header.rs>.
use std::fmt;
...
impl ForwardedHeader {
    pub fn from_info(
        ...
        if let Some(for_start) = for_start {
            ...
            let forwarded_for = forwarded[for_start..].find(';').map_or_else(
                || &forwarded[for_start..forwarded.len()],
                |for_end| &forwarded[for_start..for_start + for_end],
            );
